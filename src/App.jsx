...
function App() {
    ...
    const checkReminders = () => {
      ...
      reminders.forEach((rem) => {
        ...
        if (rem.isLoop) {
          ...
          if (elapsed > 0 && Math.floor(elapsed % intervalMs) < 1000 && !rem.triggered) shouldTrigger = true;
        } else {
          const [h, m] = rem.time.split(":").map(Number);
          const alarmDate = new Date();
          alarmDate.setHours(h, m, 0, 0);
          if (Math.abs(nowTs - alarmDate.getTime()) < 1000 && !rem.triggered) shouldTrigger = true;
        }
        if (shouldTrigger) {
          markTriggered(rem.id);
          if (!isMuted) {
            const url = (rem.method === "ring" || rem.method === "both") ? "https://actions.google.com/sounds/v1/alarms/alarm_clock.ogg" : "https://actions.google.com/sounds/v1/alarms/beep_short.ogg";
  ...
  const toggleHabit = useCallback((id) => {
    ...
    setHabits((prev) => prev.map((h) => {
      if (h.id === id) {
        ...
        if (newDates[selectedDateStr]) { delete newDates[selectedDateStr]; setXp((x) => x - 10); }
        else { newDates[selectedDateStr] = true; setXp((x) => x + 15); }
  ...
  const addNewHabit = (names, config, applyToPast) => {
    ...
    const newHabits = names.map((n, i) => {
      let hiddenDates = [];
      if (!applyToPast) {
        for (let j = 1; j <= 365; j++) hiddenDates.push(format(addDays(today, -j), "yyyy-MM-dd"));
      }
    ...
    <div className="full-screen-layout">
      ...
      <div className="main-content-area">
        ...
        <div className="scrollable-content">
          <HabitList
            selectedDate={selectedDate} visibleHabits={visibleHabits}
            toggleHabit={toggleHabit} deleteHabit={(id) => { setHabitToDeleteId(id); setShowDeleteModal(true); }}
            openAddModal={() => setShowAddForm(true)} reminders={reminders}
            onAddSchedule={(d) => setReminders([...reminders, { id: Date.now(), ...d, triggered: false }])}
            onDeleteSchedule={(id) => { if (audioRef.current) { audioRef.current.pause(); audioRef.current.currentTime = 0; } setReminders(reminders.filter((r) => r.id !== id)); }}
          />
        </div>
      </div>
      {showAddForm && <AddModal isOpen={showAddForm} onClose={() => setShowAddForm(false)} onAdd={addNewHabit} selectedDate={selectedDate} />}
      {showTimeCenter && <TimeCenter onClose={() => setShowTimeCenter(false)} />}
      <DeleteModal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} onDeleteToday={deleteForTodayOnly} onDeleteSchedule={deleteFromEveryWeekDay} onDeleteForever={deleteForever} />
    </div>
  );
}
export default App;